...
var d_direct=new Array(//保存大DIV编号的可移动位置编号
...
var d_posXY=new Array( //大DIV编号的位置，
	[0],	//同样，我们不使用第一个元素
	[0,0],	//第一个表示left，第二个表示top，比如第一块的位置为left：0，top：0
	[150,0],
	[300,0],
	[0,150],
	[150,150],
...
function move(id) {	//移动函数
	var i=1;
	for (i=1; i < 10; ++i) {//找出小DIV在大DIV中的位置
		if (d[i] == id) break;
	}
	...
	for (var k=1; k < 9;++k){//从1开始，把每个大DIV保存的编号遍历一下，判断是否完成
		if (d[k] != k ) {
			finish_flag=false;
			break;
...
function whereCanTo(cur_div) {//判断是否可移动函数，参数是大DIV的编号，不是小DIV的编号，
							//因为小DIV编号跟可以去哪没关系，小DIV是会动的
	var j=0;
	var move_flag=false;
	for (var j = 0; j < d_direct[cur_div].length; ++j) {//把所有可能去的位置循环遍历一下
...
function reset(){//重置函数
	time = 0;//把时间设置为0
	random_d();//把方块随机打乱函数
	if (pause) start();//如果暂停，则开始计时
}
//随机打乱方块函数，我们的思路是从第九块开始，随机生成一个数，然后他们两块对调一下
function random_d(){
	for(var i=9;i>1;--i){//把当前的DIV位置设置为随机产生的DIV的位置
